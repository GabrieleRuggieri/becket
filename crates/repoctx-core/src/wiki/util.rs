...
use repoctx_schema::wiki::WikiPageKind;
...
pub const PROSE_SLOT: &str = "<!-- repoctx:slot prose -->";
...
pub fn sanitize_for_context(body: &str) -> String {
    let claim_re = Regex::new(r"<!--\s*repoctx:claim[^>]*-->\n?").expect("claim regex");
...
mod tests {
    ...
    fn sanitize_strips_claims_and_placeholder() {
        let body = format!(
            "# svc\n\n<!-- repoctx:claim calls abc source=graph -->\n- **a** → **b**\n{}",
            prose_slot()
        );
        let clean = sanitize_for_context(&body);
        assert!(!clean.contains("repoctx:claim"));
