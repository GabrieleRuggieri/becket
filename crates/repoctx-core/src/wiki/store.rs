//! Read/write `.repoctx/wiki/*.md` pages.

use std::fs;
use std::path::{Path, PathBuf};

use repoctx_schema::wiki::{WikiPage, WikiPageMeta};
...
impl WikiStore {
    /// Opens the wiki store under `.repoctx/wiki/`.
...
mod tests {
    use super::*;
    use repoctx_schema::wiki::{WikiPageKind, WikiPageSource};
