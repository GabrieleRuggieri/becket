...
impl IndexStore {
    ...
    pub fn insert_edge(
        &self,
        edge: &becket_schema::artifacts::DependencyEdgeRecord,
    ) -> Result<(), StoreError> {
        self.conn.execute(
    ...
    pub fn insert_entrypoint(
        &self,
        record: &becket_schema::artifacts::EntrypointRecord,
    ) -> Result<(), StoreError> {
        self.conn.execute(
    ...
    pub fn write_artifacts(&self, paths: &crate::paths::BecketPaths) -> Result<(), StoreError> {
    ...
    pub fn find_flow_by_name(&self, name: &str) -> Result<Option<FlowRecord>, StoreError> {
        ...
        let steps = stmt
            .query_map(params![flow_id], |row| {
                Ok(becket_schema::artifacts::FlowStepRecord {
    ...
    > {
        ...
        let edges = edge_stmt
            .query_map([], |row| {
                Ok(becket_schema::artifacts::DependencyEdgeRecord {
                    id: row.get(0)?,
        ...
        let entrypoints = ep_stmt
            .query_map([], |row| {
                Ok(becket_schema::artifacts::EntrypointRecord {
                    id: row.get(0)?,
                    symbol_id: row.get(1)?,
