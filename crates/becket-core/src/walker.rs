//! Repository file walker respecting `.gitignore` and `.becketignore`.
...
impl FileWalker {
    ...
    pub fn new(root: impl AsRef<Path>) -> Self {
        ...
        let registry = GrammarRegistry::builtins()
            .load_overrides_from_file(&root.join("becket.languages.toml"))
    ...
    pub fn discover(&self) -> Result<Vec<SourceFile>, CoreError> {
        ...
        let becketignore = self.root.join(".becketignore");
        if becketignore.exists() {
            builder.add_custom_ignore_filename(".becketignore");
        }
